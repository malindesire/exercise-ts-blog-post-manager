...
Then run `nvm use` followed by `npm install` and `npm run dev`. Follow the localhost-link in the terminal to view the application. If you want some mock-up blog posts you can uncomment the the function and import in main.ts, save, comment out again and return to the application. There should now be blog posts on the page.
